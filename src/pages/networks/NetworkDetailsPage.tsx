...
export default function NetworkDetailsPage(props: PageProps) {
  ...
  const [isInitialLoad, setIsInitialLoad] = useState(true); // used to set selected gateway on initial load
  ...
  const filteredEgresses = useMemo<ExtendedNode[]>(() => {
    const filteredEgressesArray = egresses.filter(
      (egress) => egress.name?.toLowerCase().includes(searchEgress.toLowerCase()) ?? false,
    );
    // if (isInitialLoad) {
    //   setFilteredEgress(filteredEgressesArray[0] ?? null);
    //   setIsInitialLoad(false);
    // }
    return filteredEgressesArray;
  ...
  const filteredRelays = useMemo<ExtendedNode[]>(() => {
    const filteredRelays = relays.filter(
      (relay) => relay.name?.toLowerCase().includes(searchRelay.toLowerCase()) ?? false,
    );
    if (isInitialLoad) {
  ...
  const networkTabs: TabsProps['items'] = useMemo(() => {
    const tabs = [
      {
        key: 'overview',
        label: `Overview`,
        children: network && !isRefreshing ? getOverviewContent() : <Skeleton active />,
      },
      {
        key: 'hosts',
        label: `Hosts (${networkHosts.length})`,
        children: network && !isRefreshing ? getHostsContent() : <Skeleton active />,
      },
      {
        key: 'clients',
        label: `Clients (${clients.length})`,
        children: network && !isRefreshing ? getClientsContent() : <Skeleton active />,
      },
      {
        key: 'egress',
        label: `Egress (${egresses.length})`,
        children: network && !isRefreshing ? getEgressContent() : <Skeleton active />,
      },
      {
        key: 'dns',
        label: `DNS`,
        children: network && !isRefreshing ? getDnsContent() : <Skeleton active />,
      },
      {
        key: 'access-control',
        label: `Access Control`,
        children: network && !isRefreshing ? getAclsContent() : <Skeleton active />,
      },
      {
        key: 'graph',
        label: `Graph`,
        children: network && !isRefreshing ? getGraphContent() : <Skeleton active />,
      },
    ].concat(
      isServerEE
        ? [
            {
              key: 'metrics',
              label: `Metrics`,
              children: network && !isRefreshing ? getMetricsContent() : <Skeleton active />,
            },
          ]
        : [],
    );
    ...
    if (isServerEE) {
      tabs.splice(3, 0, {
        ...
        children: network && !isRefreshing ? getRelayContent() : <Skeleton active />,
      });
    }
